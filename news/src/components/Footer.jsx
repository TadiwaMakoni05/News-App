...
const Footer = () => {
  ...
  return (
    <>
    <div className={`footer grid grid-cols-2 gap-4 md:grid md:grid-cols-4 md:gap-4 p-4 m-4 justify-between items-center w-[100vw]${theme === "dark" ? "bg-black text-white" : "bg-white text-black"}`}>
        ...
        <div className='m-4 p-4'>
            ...
            <button className={`px-2 py-1 border  rounded  ${theme === "dark" ? "bg-black text-white hover:text-black hover:bg-white border-white" : "bg-white text-black hover:text-white hover:bg-black border-black"}`} >Subscribe</button>
            </p>
        </div>
    </div>
    <div className={`footer flex flex-col  md:flex md:flex-row md:justify-around md:items-center p-4  text-white gap-4 text-sm md:text-xl ${theme === "dark" ? "bg-black text-white" : "bg-white text-black"}`}>
            <p className={`text-base ${theme === "dark" ? " text-white" : " text-black"}`}>News App. Developed by: <a href='https://github.com/TadiwaMakoni05' target='_blank' className='text-blue-600 font-bold hover:text-blue-900'>Tadiwanashe Makoni </a> </p>
            <p className={`text-base ${theme === "dark" ? " text-white" : " text-black"}`}> Copyrights Reserved &copy; {year.current}</p>
    </div>
    </>
  )
}
